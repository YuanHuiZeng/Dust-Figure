  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      if (!$.isLogin) {
        ...
        if ($.isNode()) {
          await notify.BarkNotify(`${$.name}cookie已失效 - ${$.UserName}`, `京东账号${$.index} ${$.UserName}\n请重新登录获取cookie`);
        }
        continue
      }
      message = '';
...
async function jdFruit() {
  ...
  if ($.farmInfo.farmUserPro) {
    ...
    if ($.farmInfo.treeState === 2 || $.farmInfo.treeState === 3) {
      ...
      if ($.isNode()) {
        await notify.BarkNotify(`${$.name} - 账号${$.index} - ${$.nickName}水果已可领取`, `【京东账号${$.index}】${$.nickName || $.UserName}\n【提醒⏰】${$.farmInfo.farmUserPro.name}已可领取\n请去京东APP或微信小程序查看`);
      }
      return
      ...
      if ($.isNode()) {
        await notify.BarkNotify(`${$.name} - 您忘了种植新的水果`, `京东账号${$.index} ${$.nickName}\n【提醒⏰】您忘了种植新的水果\n请去京东APP或微信小程序选购并种植新的水果`);
      }
      return
    }
    await doDailyTask();
...
async function doTenWater() {
  ...
  if ($.farmTask.totalWaterTaskInit.totalWaterTaskTimes < $.farmTask.totalWaterTaskInit.totalWaterTaskLimit) {
    ...
    if (isFruitFinished) {
      ...
      $.done();
      if ($.isNode()) {
        await notify.BarkNotify(`${$.name} - 账号${$.index} - ${$.nickName || $.UserName}水果已可领取`, `京东账号${$.index} ${$.nickName}\n${$.farmInfo.farmUserPro.name}已可领取`);
      }
    }
  } else {
    console.log('\n今日已完成10次浇水任务\n');
...
async function doTenWaterAgain() {
  ...
  if (totalEnergy >= ($.farmInfo.farmUserPro.treeTotalEnergy - $.farmInfo.farmUserPro.treeEnergy)) {
    ...
    if (isFruitFinished) {
      ...
      $.done();
      if ($.isNode()) {
        await notify.BarkNotify(`${$.name} - 账号${$.index} - ${$.nickName}水果已可领取`, `京东账号${$.index} ${$.nickName}\n${$.farmInfo.farmUserPro.name}已可领取`);
      }
    }
...
async function showMsg() {
  ...
  if (ctrTemp) {
    ...
    if ($.isNode()) {
      await notify.BarkNotify(`${$.name} - 账号${$.index} - ${$.nickName}`, `${subTitle}\n${message}`);
    }
  } else {
    $.log(`\n${message}\n`);
  }
